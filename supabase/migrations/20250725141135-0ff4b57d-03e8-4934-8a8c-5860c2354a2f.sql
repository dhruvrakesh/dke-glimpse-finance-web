...
-- First, add the mapping_id column to trial_balance_entries if it doesn't exist
ALTER TABLE trial_balance_entries 
ADD COLUMN IF NOT EXISTS mapping_id UUID REFERENCES schedule3_mapping(id);
...
SELECT 
  ...
  smi.report_type,
  CASE 
    WHEN smi.category ILIKE '%ASSET%' THEN (COALESCE(tbe.debit, 0) - COALESCE(tbe.credit, 0))
    WHEN smi.category ILIKE '%LIABILITY%' OR smi.category ILIKE '%EQUITY%' THEN (COALESCE(tbe.credit, 0) - COALESCE(tbe.debit, 0))
...
LEFT JOIN schedule3_master_items smi ON sm.schedule3_item_code = smi.item_code
