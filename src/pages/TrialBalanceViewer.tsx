...
export const TrialBalanceViewer = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch trial balance entries (only existing columns)
      const { data: entriesData, error: entriesError } = await supabase
        .from('trial_balance_entries')
        .select('id, ledger_name, debit, credit, closing_balance, account_type, account_category, gpt_confidence, period_id, upload_id')
        .order('ledger_name');
